...
const EventImpact = () => {
  ...
  useEffect(() => {
    const fetchEventData = async () => {
      try {
        const response = await axios.get("http://localhost:5000/event-impact");
    ...
    const fetchPriceTrends = async () => {
      try {
        const response = await axios.get("http://localhost:5000/price-trends");
        ...
        const formattedDates = combinedDates.map((date) =>
  ...
  const cumulativeBefore = eventData.map(
    (event) => event["Cumulative Return Before"] || 0
  ); // Default to 0 if null
  const cumulativeAfter = eventData.map(
    (event) => event["Cumulative Return After"] || 0
  ); // Default to 0 if null
  ...
  const cumulativeReturnOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Cumulative Returns Before and After Events",
      },
    },
  };
        ...
        <tbody>
          {eventData.map((event, index) => (
            <tr key={index}>
              ...
              <td>{event.Date}</td>
              <td>
                {event["T-Statistic"] !== null
                  ? event["T-Statistic"].toFixed(2)
                  : "N/A"}
              </td>
              <td>
                {event["P-Value"] !== null
                  ? event["P-Value"].toFixed(4)
                  : "N/A"}
              </td>
            </tr>
          ))}
