...
const Dashboard = () => {
  ...
  const fetchPrices = async () => {
    setLoading(true);
    try {
      const priceResponse = await axios.get("http://localhost:5000/prices");
  ...
  const fetchAnalysis = async () => {
    try {
      const analysisResponse = await axios.get(
        "http://localhost:5000/analysis-metrics"
      );
  ...
  const fetchAdditionalData = async () => {
    try {
      const yearlyResponse = await axios.get(
        "http://localhost:5000/average-yearly-price"
      );
      const distributionResponse = await axios.get(
        "http://localhost:5000/price-distribution"
      );
